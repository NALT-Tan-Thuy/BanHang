...
Route::group(['prefix' => 'admin'], function(){
    ...
    Route::group(['prefix' => 'user'], function(){
        ...
        Route::get('sua/{id}', function($id){
            ...
            $tinhtp = json_decode($json);
            $user = App\User::find($id);
            return view('admin.user.sua', ['tinhtp'=> $tinhtp, 'user'=>$user]);
            //echo "<pre>";
            // var_dump($data);
            // foreach($data as $key => $value){
            //     echo $key.": ".$value->name_with_type."<br>";
            // }
        });
...
Route::get('demojson', function(){
    ...
    //         $quanhuyen = json_decode($json);
    //         $st = "";
    //         $st = $st."<h2 class=\"card-inside-title\">Quận/huyện/thị xã</h2><select class=\"form-control show-tick\"";
    //         foreach($quanhuyen as $qh){
    //             if($qh->parent_code == $str) {
    //                 $st = $st. "<option>Lựa chọn 1</option>";
    //             }
    //         }
    //         echo "$st.</select>";
    $json = File::get("hanhchinhVietNam/xa_phuong.json");
